...
{
	u16 temp,i,tempColor;
	tempColor = POINT_COLOR;
	POINT_COLOR = color; 
	for(i=0;i<=r;i++)     //从圆心到边缘循环，对称地画两条线，画之前要找到合适的长度
	{
		temp = r;                              
		while(temp*temp + i*i > r*r)          
		{
			temp--;
		}
		LCD_DrawLine(i+x,y-temp,i+x,y+temp);
		LCD_DrawLine(x-i,y-temp,x-i,y+temp);
	}
	POINT_COLOR = tempColor;
}

//写屏幕底部提示作用的汉字。（x,y）起始位置；GBK为汉字的内码；lib表示选用font.c里的哪一个字库，可自己添加字库；
//length：汉字的显示长度
void write_Chinese(u16 x,u16 y,u16 GBK,u16 color,uint8_t * lib,uint8_t length)
{
	int8_t index1,i;
	u16 j;
   ...
   byteColNum = (length/8.0) + ((length%8)?1:0);
	...
	while(GBK != Chinese64[index1])  //寻找GBK对应的汉字在Chinese64[]里的哪一个位置
	{
		index1++;
	}
...
//在给定的坐标给棋子写名字，index:要写名字的棋子的索引，lib：写名字使用的是哪一个字库
//length：写的汉字的长度
void pieceWrite(uint8_t index,uint8_t * lib,uint8_t length)
{
   ...
   byteColNum = (length/8.0) + ((length%8)?1:0);    
	byteNum = length * byteColNum;   //每一个字所用的字节数
	...
	if(camp)               //如果是黑色阵营
	{
		...
		for(j=0;j<=byteNum;j++)
		{
			for(i=0;i<=7;i++)
			{
				if(lib[index1*byteNum + j]>>(7-i) & 0x01)
				LCD_Fast_DrawPoint(ex,ey,BLACK);
				else;
				ey++;
			}
			if((j+1) % byteColNum == 0)  //一列显示完，显示下一列，从头开始
			{
				ex++;
			   ey = y-0.5*length;
			}
...
//这个函数是在LCD_DrawLine的基础上改动了一点，画斜线的时候用的这个函数,
//改动之后画的线会少一小段，以防止斜线把棋子覆盖到了
void chess_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2)
{
	...
	for(t=0;t<=distance-pieceR;t++ )//画线输出       *改动的地方就是,-pieceR,让线少画一点
	{  
		LCD_DrawPoint(uRow,uCol);//画点 
		xerr+=delta_x ; 
		yerr+=delta_y ; 
