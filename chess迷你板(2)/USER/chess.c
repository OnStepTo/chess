...
int8_t pieceTable[9][10] =           //    pieceTable[4][3]    13   将该表的xy坐标与棋盘上的位置做90°旋转，可以直接取到定对应棋子的下标
{
    {0,-1,-1,11,-1,-1,16,-1,-1,23},     //*-1代表为空无妻棋子，除-1外其他数字则表示对应棋子的下下标，位置已在上面的结构体中对应
...
int8_t cannon(int8_t x1,int8_t y1,int8_t x2,int8_t y2,int8_t Camp)
{
    ...
    if(index!=-1 && piece[index].camp == Camp)   //目的位置不为空但是是己方棋子
    return 0;
...
void change(int8_t index,int8_t x,int8_t y,ChangeBuffer *p)   //index判断是哪个棋子 x,y为棋子位置
{
    int8_t index2;
