...
int8_t gameRunFlag;
 ...
 int main(void)
 {
	...
	while (1)
	{
		getPath(&path);    //*得到路径，将其传给变量path   未返回路径则已一直卡着
		...
		if(tempPiece.judge(tempPiece.x,tempPiece.y,path.x,path.y,TURN))
		{
			...
			{
				backChange(&changeBuffer);//这样走不行，会被将军，所以撤销移  *backChange函数与change函数相对，将无效的数据改动改回
			...
			else//这样走可行
			{
				...
				if(check(tempPiece.camp,&assassin))//每次移子后判断是否将军 *check检测将军
				{
					PBout(8) = 1;     //蜂鸣器响 拉高蜂鸣器电平
					if(judgeWin(assassin))//如果将军了接着进一步判断是否将死  出1表示将死 一方胜出
					{
						POINT_COLOR = CYAN;
						...
						while(1); //*用死循环卡住，游戏结束，按RESET复位
					}
					write_Chinese(sX,sY,'将',TURN?BLACK:RED,Libbuttom,lengthCH2);   //*未将死，游戏未结束，提示将军
...
void EXTI4_IRQHandler(void)     //按键中断，悔棋处理函数
{
    ...
    if(GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_4)==0 && undoEn==1)
    {
        ...
        if(index2 != -1)//如果原来移子目的地有棋子的话，要把它画回来  *被吃了后悔棋
        {
            makePiece(index2);
        }
        else//没有的话，擦除棋子即可
